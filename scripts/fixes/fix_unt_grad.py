"""
fix_unt_grad.py - Check UNT graduate catalog and add missing doctoral programs
Run: python fix_unt_grad.py
"""
...
def get_level(raw):
    ...
    if 'grad track' in n: return "Graduate"
...
def extract_programs(soup, base_url):
    ...
    for a in soup.find_all('a', href=re.compile(r'preview_program\.php')):
        ...
        if any(rl.endswith(s) for s in SKIP_ENDS): continue
...
def best_page(root_url, root_soup):
    ...
    def rank(item):
        ...
        return 2
    sorted_navoids = sorted(navoids.items(), key=rank)

    best, best_url = [], ""
    for url, text in sorted_navoids:
        s, _ = fetch(url)
        if not s: continue
...
# ── STEP 1: Find all catalogs for UNT ──────────────────────────
print("="*60)
print("Finding all UNT catalogs from dropdown...")
...
for opt in soup.find_all('option'):
    ...
    if val and val.isdigit():
        url = f"https://catalog.unt.edu/index.php?catoid={val}"
        all_catalogs.append((val, text, url))
        print(f"  catoid={val}: {text}")

print(f"\nTotal catalogs found: {len(all_catalogs)}")

# ── STEP 2: Scrape each catalog ────────────────────────────────
all_programs = []
seen_names = set()

for catoid, cat_name, cat_url in all_catalogs:
    print(f"\n{'='*60}")
    print(f"Scraping: {cat_name} (catoid={catoid})")
    s, h = fetch(cat_url)
    if not s:
        print("  FAILED")
        continue
...
conn.commit()
conn.close()
print(f"\n✅ {ins} programs saved for University of North Texas")
